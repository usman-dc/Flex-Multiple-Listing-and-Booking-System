...
$matched_type = BookingTypePostTypeRegistry::booking_type_for_post_type( (string) $post_type );
if ( $matched_type ) {
	$type_name = (string) $matched_type['name'];
	$type_slug = (string) $matched_type['slug'];
}
...
$uid          = 'ulbm-grid-archive-' . wp_unique_id();
...
<div class="ulbm-single-listing-wrap ulbm-marketplace-ui">
	...
	<div class="container ulbm-container">
		...
		<div class="ulbm-listing-grid ulbm-marketplace-ui" id="<?php echo esc_attr( $uid ); ?>" style="<?php echo esc_attr( $grid_style ); ?>" data-type="<?php echo esc_attr( $type_slug ); ?>" data-per-page="<?php echo esc_attr( (string) $per_page ); ?>">

			<header class="ulbm-grid-hero">
				<h1 class="ulbm-grid-hero-title"><?php echo esc_html( $type_name ?: post_type_archive_title( '', false ) ); ?></h1>
			...
			<?php if ( $show_filters && $wp_query->max_num_pages > 1 ) : ?>
				<nav class="ulbm-grid-pagination mt-4" data-pages="<?php echo esc_attr( (string) $wp_query->max_num_pages ); ?>" aria-label="<?php esc_attr_e( 'Pagination', 'flex-booking-system' ); ?>">
