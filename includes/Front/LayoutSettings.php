...
final class LayoutSettings {
	...
	public static function grid_css_vars( array $overrides = array() ) {
		...
		return array(
			'--fbs-grid-gap'           => $gap . 'px',
			'--fbs-grid-padding-x'     => $pad_x . 'px',
			'--fbs-grid-padding-y'     => $pad_y . 'px',
			'--fbs-grid-margin-top'    => $margin_top . 'px',
			'--fbs-grid-margin-bottom' => $margin_bottom . 'px',
			'--fbs-grid-card-padding'  => $card_padding . 'px',
		);
	}
	...
	public static function css_vars() {
		return array_merge(
			array(
				'--fbs-container-width'       => self::container_width_px() . 'px',
				'--fbs-container-padding-x'   => '20px',
				'--fbs-container-padding-y'   => '20px',
			),
			self::grid_css_vars()
		);
	}
