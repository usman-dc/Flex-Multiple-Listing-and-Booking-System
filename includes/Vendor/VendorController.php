...
final class VendorController {
	...
	public function register_shortcodes() {
		add_shortcode( 'fbs_register', array( $this, 'shortcode_register' ) );
		add_shortcode( 'fbs_login', array( $this, 'shortcode_login' ) );
		add_shortcode( 'fbs_dashboard', array( $this, 'shortcode_dashboard' ) );
		add_shortcode( 'fbs_become_partner', array( $this, 'shortcode_become_partner' ) );
	}
	...
	public function shortcode_register() {
		if ( is_user_logged_in() ) {
			...
			ob_start();
			include FBS_PLUGIN_DIR . 'templates/public/vendor/become-partner.php';
			return $this->wrap_output( (string) ob_get_clean(), 'fbs-vendor-register-wrap' );
		}

		ob_start();
		include FBS_PLUGIN_DIR . 'templates/public/vendor/register.php';
		return $this->wrap_output( (string) ob_get_clean(), 'fbs-vendor-register-wrap' );
	}
	...
	public function shortcode_login() {
		...
		ob_start();
		include FBS_PLUGIN_DIR . 'templates/public/vendor/login.php';
		return $this->wrap_output( (string) ob_get_clean(), 'fbs-vendor-login-wrap' );
	}
	...
	public function shortcode_dashboard() {
		...
		if ( ! VendorRole::can_manage_listings() && ! current_user_can( 'manage_options' ) ) {
			return $this->wrap_output(
				...
				'fbs-vendor-dashboard-wrap'
			);
		}

		$user_id = get_current_user_id();
		$tab     = isset( $_GET['fbs_tab'] ) ? sanitize_key( wp_unslash( $_GET['fbs_tab'] ) ) : 'overview';
		$allowed = array( 'overview', 'listings', 'add', 'bookings', 'profile' );
		if ( ! in_array( $tab, $allowed, true ) ) {
			$tab = 'overview';
		}

		$fbs_vendor_user      = wp_get_current_user();
		$fbs_vendor_tab       = $tab;
		$fbs_vendor_listings  = VendorListingService::get_listings( $user_id );
		$fbs_vendor_bookings  = VendorListingService::get_vendor_bookings( $user_id, 25 );
		$fbs_booking_types    = ( new \FlexBooking\Booking\BookingTypeRepository() )->get_all();
		$fbs_vendor_record    = ( new VendorRepository() )->get_by_user_id( $user_id );
		$fbs_edit_listing_id  = isset( $_GET['edit'] ) ? absint( $_GET['edit'] ) : 0;
		$fbs_edit_listing     = null;
		if ( $fbs_edit_listing_id && VendorListingService::user_owns_listing( $user_id, $fbs_edit_listing_id ) ) {
			$fbs_edit_listing = get_post( $fbs_edit_listing_id );
			$tab              = 'add';
			$fbs_vendor_tab   = 'add';
		}

		ob_start();
		include FBS_PLUGIN_DIR . 'templates/public/vendor/dashboard.php';
		return $this->wrap_output( (string) ob_get_clean(), 'fbs-vendor-dashboard-wrap' );
	}
	...
	public function shortcode_become_partner( $atts ) {
		...
		ob_start();
		?>
		<div class="fbs-become-partner border rounded p-4 bg-light text-center">
		...
		<?php
		return $this->wrap_output( (string) ob_get_clean(), 'fbs-become-partner-wrap' );
	}
	...
	private function redirect_notice( $url, $msg, $label ) {
		return $this->wrap_output(
			sprintf(
				'<div class="fbs-auth-notice alert alert-info"><p class="mb-2">%s</p><a class="btn btn-primary btn-sm" href="%s">%s</a></div>',
				esc_html( $msg ),
				esc_url( $url ),
				esc_html( $label )
			),
			'fbs-auth-notice-wrap'
		);
	}
}
