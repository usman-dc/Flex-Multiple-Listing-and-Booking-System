...
final class BookingAdminUpdater {
	...
	public static function update( $booking_id, $new_status, $new_payment, $send_notification ) {
		...
		if ( ! $row ) {
			return new \WP_Error( 'fbs_not_found', __( 'Booking not found.', 'flex-booking-system' ), array( 'status' => 404 ) );
		}
		...
		if ( null !== $new_status && '' !== $new_status ) {
			...
			if ( ! in_array( $st, self::booking_statuses(), true ) ) {
				return new \WP_Error( 'fbs_bad_status', __( 'Invalid booking status.', 'flex-booking-system' ), array( 'status' => 400 ) );
			}
			if ( $st !== $old_st ) {
				$data['status'] = $st;
			}
		...
		if ( null !== $new_payment && '' !== $new_payment ) {
			...
			if ( ! in_array( $py, self::payment_statuses(), true ) ) {
				return new \WP_Error( 'fbs_bad_payment', __( 'Invalid payment status.', 'flex-booking-system' ), array( 'status' => 400 ) );
			}
			if ( $py !== $old_pay ) {
		...
		if ( empty( $data ) ) {
			return new \WP_Error( 'fbs_no_change', __( 'Nothing to update.', 'flex-booking-system' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $fresh ) {
			return new \WP_Error( 'fbs_reload', __( 'Update failed to persist.', 'flex-booking-system' ), array( 'status' => 500 ) );
		}
